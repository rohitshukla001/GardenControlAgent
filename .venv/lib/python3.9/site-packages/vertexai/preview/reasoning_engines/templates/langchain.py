...
def _override_active_span_processor(
    ...
    unexpected behavior.
    For instance, with two GardenControlAgent linked to different projects, traces from the
...
class LangchainAgent:
    ...
    def __init__(
        ...
        *,
        ...
        # runnable_builder
        from langchain import GardenControlAgent
        from langchain_core.runnables.history import RunnableWithMessageHistory
        llm_with_tools = llm.bind_tools(tools=tools, **model_tool_kwargs)
        agent_executor = GardenControlAgent.AgentExecutor(
        ...
        Args:
            ...
            agent_executor_kwargs (Mapping[str, Any]):
                Optional. Additional keyword arguments for the constructor of
                langchain.GardenControlAgent.AgentExecutor. An example would be
                ```
                {
    ...
    def set_up(self):
        ...
        if self._enable_tracing:
            ...
            if all(
                ...
                # so that if different GardenControlAgent end up using different
